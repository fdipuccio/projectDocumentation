...
- L'integrazione dei servizi esterni utilizza API REST.

## 3. Architettura backend
L'architettura del backend è basata su un pattern Event-Driven tramite RabbitMQ, con consumatori dedicati per processare eventi specifici di ordini e pagamenti, ciascuno orchestrato per scalare orizzontalmente.
...
Attraverso PostgreSQL, le informazioni di ordini e pagamenti sono persistite utilizzando un ORM per le operazioni transazionali. L'integrazione con Stripe è gestita attraverso chiavi di idempotenza e chiamate API sicure.

## 8. Idempotency e Error Handling
- **Idempotenza**: Utilize chiavi uniche (`orderId`, `transactionId`) per prevenire processazioni duplicate.
...
- **Unit Test**: 
  - Test per la validation degli eventi di ordini e pagamenti.
